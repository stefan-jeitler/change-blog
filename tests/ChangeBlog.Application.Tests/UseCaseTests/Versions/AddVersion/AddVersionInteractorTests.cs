...
namespace ChangeBlog.Application.Tests.UseCaseTests.Versions.AddVersion;
...
public class AddVersionInteractorTests
{
    ...
    [Fact]
    public async Task AddVersion_ProductIsFreezed_ProductFreezedOutput()
    {
        // arrange
        var changeLogLines = new List<ChangeLogLineRequestModel>
        {
        ...
        // assert
        _outputPortMock.Verify(m => m.RelatedProductFreezed(It.Is<Guid>(x => x == TestAccount.Product.Id)),
            Times.Once);
    }
    ...
    public async Task AddVersion_TooManyChangeLogLines_TooManyLinesOutput()
    {
        ...
        // assert
        _outputPortMock.Verify(m =>
            m.TooManyLines(It.Is<int>(x => x == Domain.ChangeLog.ChangeLogs.MaxLines)), Times.Once);
    }
