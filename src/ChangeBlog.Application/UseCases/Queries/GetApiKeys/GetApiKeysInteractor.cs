...
public class GetApiKeysInteractor : IGetApiKeys
{
    ...
    public GetApiKeysInteractor(IUserDao userDao, IUserApiKeysDao userApiKeysDao)
    {
        _userDao = userDao ?? throw new ArgumentNullException(nameof(userDao));
        _userApiKeysDao = userApiKeysDao;
    }
    ...
    public async Task<IList<ApiKeyResponseModel>> ExecuteAsync(Guid userId)
    {
        ...
        var apiKeys = await _userApiKeysDao.GetUserApiKeysAsync(userId);
