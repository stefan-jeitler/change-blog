...
namespace ChangeBlog.Application.Tests.UseCaseTests.ChangeLogs.AddPendingChangeLogLine;
...
public class AddPendingChangeLogLineInteractorTests
{
    ...
    {
        return new AddPendingChangeLogLineInteractor(_fakeProductDao, _fakeChangeLogDao,
    ...
    public async Task AddPendingLine_MaxLinesReached_TooManyLinesOutput()
    {
        ...
        // assert
        _outputPortMock.Verify(m => m.TooManyLines(It.Is<int>(x => x == Domain.ChangeLog.ChangeLogs.MaxLines)),
            Times.Once);
    }
