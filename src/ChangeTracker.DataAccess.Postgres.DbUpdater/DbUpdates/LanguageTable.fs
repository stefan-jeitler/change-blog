...
let private addGermanSql = "INSERT INTO language VALUES('de', 'German', now()) ON CONFLICT (code) DO NOTHING"

let private addEnglishSql = "INSERT INTO language VALUES('en', 'English', now()) ON CONFLICT (code) DO NOTHING"

let addLanguageTableWithBasicLanguages (connection: IDbConnection) = 
    connection.Execute(createLanguageTableSql) |> ignore
